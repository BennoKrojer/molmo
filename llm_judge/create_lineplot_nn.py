#!/usr/bin/env python3
"""
Create a line plot visualization of LLM judge interpretability results for nearest neighbors.
...
def load_results(results_dir):
    ...
    for results_file in results_dir.glob("**/results_*.json"):
        ...
        # Path format: llm_judge_{llm}_{vision_encoder}_layer{layer}_{suffix}_cropped/results_validation.json
        # or: llm_judge_{llm}_{vision_encoder}_seed{seed}_layer{layer}_{suffix}_cropped/results_validation.json
        path_str = str(results_file)
        
        # Extract layer from path (more reliable for NN since it's in the folder name)
        ...
        # Pattern: llm_judge_{llm}_{vision_encoder}(_seed10)?_layer{N}
...
def create_lineplot(data, output_path, title='Nearest Neighbors Interpretability Across Layers (MLLM Judge)', 
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Create line plot from LLM judge nearest neighbors results')
    parser.add_argument(
        '--results-dir',
        type=str,
        default='analysis_results/llm_judge_nearest_neighbors',
        help='Directory containing LLM judge nearest neighbors results'
    )
    parser.add_argument(
        '--output',
        type=str,
        default='analysis_results/llm_judge_nearest_neighbors/lineplot_interpretability.pdf',
    ...
    create_lineplot(data, output_path, 
                   title='Nearest Neighbors Interpretability Across Layers (MLLM Judge)',
