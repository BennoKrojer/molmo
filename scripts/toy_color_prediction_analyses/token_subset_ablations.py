...
def sample_token_combinations(n: int, k: int, exclude=None):
    """Randomly sample *n* k‑sized combos from pool (optionally excluding some)."""
    pool = [i for i in all_token_indices() if exclude is None or i not in exclude]
...
def evaluate_subset(model, preprocessor, dataset, prompt, removed_tokens, num_images):
    """Return accuracy when *removed_tokens* are **masked out** (i.e. *not* fed)."""

    kept_indices = sorted(set(range(TOTAL_TOKENS)) - set(removed_tokens))
    correct = 0
...
def main():
    ...
    for pair in tqdm(pair_list, desc="2‑token"):
        ...
        results["token_pairs_removed"][str(pair)] = acc
    ...
    for tri in tqdm(triplet_list, desc="3‑token"):
        ...
        results["token_triplets_removed"][str(tri)] = acc

    # save --------------------------------------------------------------------
    out = Path(f"analysis_results/token_removal_study_{split}_{ckpt_name}.json")
