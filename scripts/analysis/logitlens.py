"""Logit Lens Analysis for Visual Tokens
...
Usage:
    torchrun --nproc_per_node=2 scripts/analysis/logitlens.py --ckpt-path <path> --num-images 100
"""
        ...
        with torch.inference_mode():
            with torch.autocast("cuda", enabled=True, dtype=torch.float16):
                ...
                for layer_idx in layers_to_process:
                    ...
                    visual_features_normed = model.module.transformer.ln_f(visual_features)
                    logits = model.module.transformer.ff_out(visual_features_normed)
                    
                    if model.module.config.scale_logits:
                        logits = logits / math.sqrt(model.module.config.d_model)
