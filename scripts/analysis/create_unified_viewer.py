...
# Import shared viewer functions
import sys
...
# All shared utility functions now imported from viewer_lib
...
<body>
    <div class="container">
        ...
        <div class="info">
            ...
            <p>This unified interface provides interactive access to four types of analyses across multiple model combinations:</p>
            <ul>
                <li><strong>LatentLens:</strong> Nearest contextual embeddings (words in real captions)</li>
                ...
                <li><strong>Patchscopes:</strong> Free-form descriptions generated by patching visual tokens</li>
            </ul>
...
def load_all_analysis_data(analysis_results: Dict, split: str, num_images: int) -> Dict:
    ...
    log.info(f"    Loading {len(analysis_results['contextual_cc'])} LatentLens (CC) files...")
    t0 = time.time()
    ...
    log.info(f"    ⏱️  LatentLens (CC): {time.time() - t0:.2f}s")
    ...
    log.info(f"    Loading {len(analysis_results['contextual_vg'])} LatentLens files...")
    t0 = time.time()
    ...
    log.info(f"    ⏱️  LatentLens: {time.time() - t0:.2f}s")
    ...
    if analysis_results.get("patchscopes"):
        log.info(f"    Loading {len(analysis_results['patchscopes'])} Patchscopes files...")
        t0 = time.time()
        for layer, json_path in analysis_results["patchscopes"].items():
            try:
                with open(json_path, 'r') as f:
                    full_data = json.load(f)
                    results = full_data.get("results", [])
                    all_data["patchscopes"][layer] = results[:num_images]
            except Exception as e:
                log.warning(f"Could not load patchscopes data for layer {layer}: {e}")
        log.info(f"    ⏱️  Patchscopes: {time.time() - t0:.2f}s")
    else:
        log.info(f"    No patchscopes data available for this model")
...
def get_image_data_from_cache(all_data_cache: Dict, image_idx: int) -> Dict:
    ...
    for analysis_type in ["nn", "logitlens", "contextual_cc", "contextual_vg", "patchscopes"]:
...
def create_unified_image_viewer(output_dir: Path, checkpoint_name: str, llm: str, ve: str,
    ...
    patchscopes_layers = sorted(all_data["patchscopes"].keys())

    if not nn_layers and not logit_layers and not ctx_cc_layers and not ctx_vg_layers and not patchscopes_layers:
        if image_idx % 50 == 0:
    ...
    for analysis_type in ["nn", "logitlens", "contextual_vg", "contextual_cc", "patchscopes"]:
    ...
    # Process LatentLens data - VG (Visual Genome corpus)
    ...
    # Process LatentLens data - CC (Conceptual Captions corpus, legacy)
    ...
    # Process Patchscopes data
    for layer, image_data in all_data["patchscopes"].items():
        unified_patch_data["patchscopes"][layer] = {}
        chunks = image_data.get("chunks", [])
        for chunk in chunks:
            for patch in chunk.get("patches", []):
                patch_idx = patch.get("patch_idx", -1)
                ...
                # Patchscopes has single description instead of top-5
                description = escape_for_html(patch.get("description", ""))

                unified_patch_data["patchscopes"][layer][patch_idx] = {
                    "row": row,
                    "col": col,
    ...
    html_content = create_unified_html_content(
        ...
        nn_layers, logit_layers, ctx_cc_layers, ctx_vg_layers, patchscopes_layers,
...
def create_unified_html_content(image_idx: int, image_base64: str, ground_truth: str,
    ...
    Args:
        ...
        display_name: Optional. Custom display name for the model (used for ablations).
                      If not provided, computed from llm and ve.
    """
    ...
    # Compute display name: use custom if provided, otherwise compute from llm/ve
    if display_name is None:
        display_name = f"{LLM_DISPLAY_NAMES[llm]} + {VISION_ENCODER_DISPLAY_NAMES[ve]}"

    # Find all unique layers across all analysis types (including patchscopes)
    all_layers = sorted(set(nn_layers + logit_layers + ctx_cc_layers + ctx_vg_layers + patchscopes_layers))
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Image {image_idx:04d} - {display_name}</title>
    <style>
        body {{
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
            margin: 0;
        ...
        .analysis-column.patchscopes-column h3 {{ color: #9c27b0; border-color: #9c27b0; }}
        .analysis-results {{
            flex: 1;
            overflow-y: auto;
            max-height: 500px;
        ...
        .patchscopes-column .result-item {{ border-left: 3px solid #9c27b0; }}
        .result-header {{
            font-weight: 600;
            margin-bottom: 4px;
            font-size: 12px;
        ...
        .patchscopes-column .result-header {{ color: #9c27b0; }}
        .patchscopes-indicator {{
            position: absolute;
            top: 2px;
            right: 2px;
            width: 8px;
...
<body>
    <div class="container">
        <div class="breadcrumb">
            ...
            <span>|</span>
            <a href="index.html">← {display_name}</a>
        </div>
    ...
    html += f'''
                ...
                <div class="instructions">
                    <p><strong>Instructions:</strong> Click on any patch to see all four analyses side-by-side.</p>
                    <p><strong>Grid:</strong> {grid_size}×{grid_size} patches ({patches_per_chunk} total)</p>
                    <p><strong>Light green (1.5-1.8×):</strong> Maybe interpretable | <strong>Dark green (≥1.8×):</strong> Likely interpretable | <strong style="color:#9c27b0;">●</strong> Has Patchscopes</p>
            ...
            <div class="info-section">
                ...
                <div class="analysis-columns">
                    ...
                    <div class="analysis-column patchscopes-column">
                        <h3>🔬 Patchscopes</h3>
                        <div class="analysis-results" id="patchscopesResults">
                            <div class="empty-state">Select a patch</div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        const availableLayers = {{
            ...
            "patchscopes": {json.dumps(patchscopes_layers)}
        }};
        ...
        function createPatches() {{
            ...
            // Find available data for current layer (try NN first, then logit, then contextual VG, then CC, then patchscopes)
            ...
            }} else if (allData.patchscopes[layerKey]) {{
                referenceData = allData.patchscopes[layerKey];
            }}
            ...
            Object.entries(referenceData).forEach(([patchIdx, patchData]) => {{
                ...
                // Add purple dot indicator if this patch has Patchscopes data
                if (allData.patchscopes[layerKey] && allData.patchscopes[layerKey][patchIdx]) {{
                    const indicator = document.createElement('div');
                    indicator.className = 'patchscopes-indicator';
                    indicator.title = 'Has Patchscopes description';
                    patchDiv.appendChild(indicator);
                }}
        ...
        function updateResults(patchIdx) {{
            ...
            const patchscopesData = allData.patchscopes[layerKey] && allData.patchscopes[layerKey][patchIdx];
            const interpData = interpretabilityData[patchIdx];

            // Update patch info at top
            const patchInfo = document.getElementById('patchInfo');
            if (nnData || logitData || ctxVgData || ctxCcData || patchscopesData) {{
                const row = (nnData || logitData || ctxVgData || ctxCcData || patchscopesData).row;
                const col = (nnData || logitData || ctxVgData || ctxCcData || patchscopesData).col;
            ...
            // Update LatentLens column - show VG first, then CC (legacy)
            ...
            // Update Patchscopes column
            const patchscopesResults = document.getElementById('patchscopesResults');
            if (patchscopesData && patchscopesData.description) {{
                // Patchscopes shows a single description (not top-5)
                patchscopesResults.innerHTML = `<div class="result-item">
                    <div class="result-header">Generated Description</div>
                    <div class="result-content" style="font-style: italic; line-height: 1.4;">${{patchscopesData.description || '(empty)'}}</div>
                </div>`;
            }} else {{
                patchscopesResults.innerHTML = '<div class="no-data">No data for layer ${{currentLayer}}</div>';
            }}
        }}
        ...
        document.addEventListener('click', (e) => {{
            ...
            if (!e.target.classList.contains('patch') && 
                ...
                if (activePatchDiv) {{
                    ...
                    document.getElementById('patchscopesResults').innerHTML = '<div class="empty-state">Select a patch</div>';
                }}
            }}
        }});
    ...
    for llm in LLMS:
        for ve in VISION_ENCODERS:
            ...
            has_results = (len(analysis_results["nn"]) > 0 or
                          ...
                          len(analysis_results["contextual_vg"]) > 0 or
                          len(analysis_results["patchscopes"]) > 0)
            ...
            if has_results:
                log.info(f"  Found: NN={len(analysis_results['nn'])}, "
                        ...
                        f"Patchscopes={len(analysis_results['patchscopes'])}")
