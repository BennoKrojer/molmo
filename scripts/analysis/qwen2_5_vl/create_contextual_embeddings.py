#!/usr/bin/env python3
"""
Create contextual embeddings from Qwen2.5-VL's LLM backbone (text-only, no vision).

This script is similar to create_contextual_embeddings.py but specifically for Qwen2.5-VL.
Since Qwen2.5-VL uses a finetuned version of Qwen2, we need to extract embeddings from
the actual Qwen2.5-VL model to get properly aligned contextual embeddings.

Key differences from the vanilla Qwen2 version:
1. Uses Qwen2_5_VLForConditionalGeneration model
...
Usage:
    # Test with small number of captions (needs 4 GPUs for 32B model)
    CUDA_VISIBLE_DEVICES=0,1,2,3 python scripts/analysis/qwen2_5_vl/create_contextual_embeddings.py \
        --num-captions 1000 --layers 1 2 4 8 16 32 48 56 62 63

    # Full extraction (32B model needs device_map=auto across 4 GPUs, so no per-GPU sharding):
    CUDA_VISIBLE_DEVICES=0,1,2,3 python scripts/analysis/qwen2_5_vl/create_contextual_embeddings.py \
        --dataset vg --num-captions -1 --embedding-dtype float8 \
        --layers 1 2 4 8 16 32 48 56 62 63

    # Then build caches:
    python scripts/analysis/qwen2_5_vl/create_contextual_embeddings.py --merge-shards --num-shards 1 --dataset vg
"""
...
def extract_contextual_embeddings_qwen2vl(
    ...
    """
    Extract contextual embeddings from Qwen2.5-VL's LLM backbone (text-only).
    
    This processes pure text through Qwen2.5-VL without any vision inputs,
    ...
    for batch_idx in tqdm(range(0, len(captions), batch_size), desc="Processing batches"):
        ...
        with torch.no_grad():
            with torch.autocast("cuda", enabled=True, dtype=torch.float16):
                # Access the LLM directly — Qwen2.5-VL nests it under model.model.language_model
                # (Qwen2-VL used model.model directly)
...
def main():
    parser = argparse.ArgumentParser(description="Create contextual embeddings from Qwen2.5-VL LLM backbone")
    ...
    print("=" * 70)
    print(f"QWEN2.5-VL CONTEXTUAL EMBEDDINGS EXTRACTION{shard_info}")
    print("=" * 70)
    ...
    metadata = {
        ...
        'model_type': 'Qwen2.5-VL (LLM backbone only, no vision)',
