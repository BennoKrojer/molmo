#!/usr/bin/env python3
"""
Contextual nearest neighbors for Qwen2.5-VL across all layers (single GPU version).

This is the Qwen2.5-VL equivalent of scripts/analysis/contextual_nearest_neighbors_allLayers_singleGPU.py
...
Usage:
    CUDA_VISIBLE_DEVICES=0 python scripts/analysis/qwen2_vl/contextual_nearest_neighbors_allLayers_singleGPU.py \
        --contextual-dir molmo_data/contextual_llm_embeddings_vg/Qwen_Qwen2.5-VL-7B-Instruct \
...
def extract_vision_features_all_layers(model, processor, image, prompt, visual_layers, device):
    """
    Extract vision token features from specified layers of Qwen2.5-VL.
...
def main():
    parser = argparse.ArgumentParser(description="Contextual nearest neighbors for Qwen2.5-VL")
    ...
    parser.add_argument("--contextual-dir", type=str, required=True,
                       help="Directory with contextual embeddings (e.g., molmo_data/contextual_llm_embeddings_vg/Qwen_Qwen2.5-VL-7B-Instruct)")
    ...
    parser.add_argument("--visual-layer", type=str, default="0,1,8,16,32,48,56,62,63",
    ...
    print("=" * 70)
    print("QWEN2.5-VL CONTEXTUAL NEAREST NEIGHBORS")
    print("=" * 70)
