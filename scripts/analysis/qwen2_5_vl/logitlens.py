#!/usr/bin/env python3
"""
LogitLens Analysis for Qwen2.5-VL Vision Tokens
...
Usage:
    CUDA_VISIBLE_DEVICES=0 python scripts/analysis/qwen2_vl/logitlens.py \
        --num-images 100 --layers "0,8,16,24,27" --output-dir analysis_results/logit_lens/qwen2_5_vl
"""
...
# Import shared preprocessing (SINGLE SOURCE OF TRUTH for Qwen2.5-VL)
...
IMAGE_PAD_TOKEN_ID = 151655  # <|image_pad|> token ID in Qwen2.5-VL
...
def extract_logitlens(model, processor, image, prompt, device, layers_to_analyze, top_k=5):
    ...
    # Qwen2.5-VL uses: model.lm_head (no separate ln_f before lm_head typically)
    ...
    for layer_idx in layers_to_analyze:
        ...
        # Qwen2.5-VL nests LLM under model.model.language_model (unlike Qwen2-VL's model.model)
        if hasattr(model.model, 'language_model') and hasattr(model.model.language_model, 'norm'):
            vision_hs_normed = model.model.language_model.norm(vision_hs)
...
def main():
    parser = argparse.ArgumentParser(description="LogitLens for Qwen2.5-VL vision tokens")
    parser.add_argument("--model-name", type=str, default="Qwen/Qwen2.5-VL-32B-Instruct")
    parser.add_argument("--num-images", type=int, default=100)
    parser.add_argument("--split", type=str, default="validation")
    parser.add_argument("--output-dir", type=str, default="analysis_results/logit_lens/qwen2_5_vl")
    parser.add_argument("--layers", type=str, default="0,1,2,4,8,16,32,48,56,62,63",
    ...
    print("=" * 70)
    print("QWEN2.5-VL LOGITLENS ANALYSIS")
    print("=" * 70)
