#!/usr/bin/env python3
"""
EmbeddingLens (Static Nearest Neighbor) Analysis for LLaVA-1.5-7B Vision Tokens.

Finds the nearest vocabulary embeddings for vision tokens at each LLM layer.

Usage:
    CUDA_VISIBLE_DEVICES=0 python scripts/analysis/llava_1_5/nearest_neighbors.py \
...
def find_vision_token_range(input_ids, hidden_states_seq_len=None):
    ...
    if num_vision != NUM_VISION_TOKENS:
        print(f"  Warning: found {num_vision} image tokens, expected {NUM_VISION_TOKENS}")
...
def main():
    parser = argparse.ArgumentParser(description="EmbeddingLens for LLaVA-1.5-7B")
    parser.add_argument("--num-images", type=int, default=100)
    parser.add_argument("--split", type=str, default="validation")
    parser.add_argument("--output-dir", type=str,
                       default="analysis_results/nearest_neighbors/llava_1_5")
    parser.add_argument("--layers", type=str, default="0,1,2,4,8,16,24,30,31")
