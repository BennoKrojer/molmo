#!/usr/bin/env python3
"""
Create contextual embeddings from LLaVA-1.5-7B's finetuned Vicuna LLM backbone (text-only).
...
Usage:
    ...
    CUDA_VISIBLE_DEVICES=0 python scripts/analysis/llava_1_5/create_contextual_embeddings.py \
        --num-captions 1000 --layers 1 2 4 8 16 24 30 31

    # Full extraction with 8 GPUs in parallel:
    for i in {0..7}; do
        CUDA_VISIBLE_DEVICES=$i python scripts/analysis/llava_1_5/create_contextual_embeddings.py \
            --dataset vg --num-captions -1 --shard $i --num-shards 8 --embedding-dtype float8 &
    done

    # Merge shards and build caches:
    python scripts/analysis/llava_1_5/create_contextual_embeddings.py --merge-shards --num-shards 8 --dataset vg
"""
...
def load_llava_model():
    """
    Load LLaVA-1.5-7B and return a text-only forward function.

    LLaVA's architecture: LlavaForConditionalGeneration contains:
    ...
    print(f"Loaded LLaVA-1.5-7B: {num_layers} layers, hidden_size={hidden_size}")
...
def main():
    ...
    run_extraction(
        ...
        model_type_description="LLaVA-1.5-7B (finetuned Vicuna/LLaMA2 LLM backbone, text-only)",
    )
