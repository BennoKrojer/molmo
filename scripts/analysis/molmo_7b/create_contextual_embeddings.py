#!/usr/bin/env python3
"""
Create contextual embeddings from Molmo-7B-D's finetuned Qwen2 LLM backbone (text-only).

Molmo-7B-D uses a finetuned Qwen2-7B as its LLM backbone. We extract contextual
...
Usage:
    ...
    CUDA_VISIBLE_DEVICES=0 python scripts/analysis/molmo_7b/create_contextual_embeddings.py \
        --num-captions 1000 --layers 1 2 4 8 16 24 26 27

    # Full extraction with 8 GPUs in parallel:
    for i in {0..7}; do
        CUDA_VISIBLE_DEVICES=$i python scripts/analysis/molmo_7b/create_contextual_embeddings.py \
            --dataset vg --num-captions -1 --shard $i --num-shards 8 --embedding-dtype float8 &
    done

    # Merge shards and build caches:
    python scripts/analysis/molmo_7b/create_contextual_embeddings.py --merge-shards --num-shards 8 --dataset vg
"""
...
def load_molmo_model():
    """
    Load Molmo-7B-D and return a text-only forward function.
    ...
    print(f"Loaded Molmo-7B-D: {num_layers} layers, hidden_size={model.config.hidden_size}")
...
def main():
    ...
    run_extraction(
        ...
        model_type_description="Molmo-7B-D (finetuned Qwen2 LLM backbone, text-only)",
    )
