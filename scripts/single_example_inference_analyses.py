"""Run inference analyses on multiple examples with a fixed prompt."""
import logging
import sys
import json
    ...
    for i in tqdm(range(num_images)):
        ...
        with torch.inference_mode():
            with torch.autocast("cuda", enabled=True, dtype=torch.float16):
                # Move data to GPU with explicit dtype
                images_tensor = torch.tensor(batch.get("images"), dtype=torch.float16).unsqueeze(0).cuda()
                image_masks_tensor = torch.tensor(batch.get("image_masks"), dtype=torch.float16).unsqueeze(0).cuda() if batch.get("image_masks") is not None else None
                ...
                # Get token embeddings from the model and ensure same dtype
                token_embeddings = model.transformer.wte.embedding.to(dtype=torch.float16)
                ...
                # Scale values before normalization to prevent overflow
                image_features_scale = torch.max(torch.abs(image_features_reshaped)).clamp(min=1e-8)
                token_embeddings_scale = torch.max(torch.abs(token_embeddings)).clamp(min=1e-8)
                
                image_features_scaled = image_features_reshaped / image_features_scale
                token_embeddings_scaled = token_embeddings / token_embeddings_scale
                
                # Normalize the scaled embeddings
                image_features_norm = torch.nn.functional.normalize(image_features_scaled, dim=-1, eps=1e-8)
                token_embeddings_norm = torch.nn.functional.normalize(token_embeddings_scaled, dim=-1, eps=1e-8)
