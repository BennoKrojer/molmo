#!/usr/bin/env python3
"""
Analyze the occurrence of visual attribute words (colors, shapes, textures) in nearest neighbors across VISUAL layers.
...
def load_visual_attribute_counts(contextual_nn_dir, static_nn_dir, llm, vision_encoder):
    """
    Load all NN results and count occurrences of visual attribute words per VISUAL layer.
    ...
    Returns:
        dict: {visual_layer: {'color': count, 'shape': count, 'texture': count, 'total_words': count, 'total_tokens': count}}
    """
    ...
    if static_nn_dir and 0 not in data:
        ...
        if static_dir.exists():
            ...
            if nn_file.exists():
                ...
                print(f"    ✓ Layer 0: {data[0]['total_tokens']} tokens, {data[0]['total_words']} words")
            else:
...
def create_visualization(data, output_path, llm, vision_encoder):
    ...
    print("="*115)
    print(f"{'Visual Layer':<15}{'Color %':<15}{'Shape %':<15}{'Texture %':<15}{'Total Words':<18}{'Total Tokens':<15}")
    print("-" * 115)
    for i, layer in enumerate(layers):
        print(f"{layer:<15}{color_pct[i]:<15.2f}{shape_pct[i]:<15.2f}"
...
def main():
    parser = argparse.ArgumentParser(
        description='Analyze visual attribute word occurrence across visual layers'
    )
    parser.add_argument(
        '--llm',
        type=str,
    ...
    if args.combined:
        ...
        # Try to load from cache - but invalidate cache since we changed the logic
        all_data_dict = {}
        # Don't use cache for now since we changed the logic
        ...
        print(f"Loading data from JSON files (this may take 10+ minutes)...")
