...
#   4. LLM Judge for NN, LogitLens, Contextual - RUN IN PARALLEL
#      - 3 types × 100 images × 9 layers = 2,700 API calls total
#
# Total API calls: ~2,700 (with --skip-llm-judge: 0)
# Estimated time: ~4h (GPU sequential, API parallel)
# =============================================================================
...
# =============================================================================
# Step 4: LLM Judge for ALL analysis types (NN, LogitLens, Contextual) - PARALLEL
# =============================================================================
if [ "$SKIP_LLM_JUDGE" = true ]; then
    ...
    echo "[4/4] Running LLM Judge for ALL analysis types (parallel)..."
    echo "  3 types × $JUDGE_IMAGES images × ${#JUDGE_LAYERS[@]} layers = $((3 * JUDGE_IMAGES * ${#JUDGE_LAYERS[@]})) total API calls"
    echo "  Running NN, LogitLens, and Contextual in parallel (9 layers each)"
    echo ""
    
    mkdir -p logs/llm_judge_qwen2vl
    
    if [ "$DRY_RUN" = true ]; then
        echo "  [DRY RUN] Would run 3 parallel processes:"
        echo "    1. NN: 9 layers sequentially"
        echo "    2. LogitLens: 9 layers sequentially"
        echo "    3. Contextual: 9 layers sequentially"
    else
        # Function to run all layers for one analysis type
        run_nn_judge() {
            for layer in "${JUDGE_LAYERS[@]}"; do
                echo "[NN] Layer $layer..."
                python llm_judge/run_single_model_with_viz.py \
                    ...
                    2>&1 | tee -a logs/llm_judge_qwen2vl/nn_layer${layer}.log || echo "[NN] Layer $layer failed"
            done
            echo "[NN] ✓ All layers done"
        }
        ...
        run_logitlens_judge() {
            for layer in "${JUDGE_LAYERS[@]}"; do
                ...
                python llm_judge/run_single_model_with_viz_logitlens.py \
                    ...
                    2>&1 | tee -a logs/llm_judge_qwen2vl/logitlens_layer${layer}.log || echo "[LogitLens] Layer $layer failed"
            done
            echo "[LogitLens] ✓ All layers done"
        }
        
        run_contextual_judge() {
            for layer in "${JUDGE_LAYERS[@]}"; do
                echo "[Contextual] Layer $layer..."
                python llm_judge/run_single_model_with_viz_contextual.py \
                    --model qwen2-vl \
                    --layer $layer \
                    --num-images $JUDGE_IMAGES \
                    --num-samples 1 \
                    2>&1 | tee -a logs/llm_judge_qwen2vl/contextual_layer${layer}.log || echo "[Contextual] Layer $layer failed"
            done
            echo "[Contextual] ✓ All layers done"
        }
        
        # Export functions and variables for subshells
        export -f run_nn_judge run_logitlens_judge run_contextual_judge
        export JUDGE_LAYERS JUDGE_IMAGES
        
        # Run all 3 in parallel
        echo "  Starting 3 parallel LLM Judge processes..."
        run_nn_judge &
        PID_NN=$!
        run_logitlens_judge &
        PID_LOGITLENS=$!
        ...
        echo "  PIDs: NN=$PID_NN, LogitLens=$PID_LOGITLENS, Contextual=$PID_CONTEXTUAL"
        echo "  Logs: logs/llm_judge_qwen2vl/*.log"
        echo ""
        ...
        echo "  ✓ All LLM Judge processes completed"
    fi
fi
echo ""
